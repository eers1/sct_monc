...
def add_diurnal(ds, ax, ylims, alpha=None):
    ...
    for s in splits:
        plot_obj = ax.fill_between(s, ylims[0], ylims[1],color='grey', alpha=alpha, edgecolor=None)
    return plot_obj
...
def load_lwp_cmap():
    ...
    cdict = {'red':   [[0.0,  R, R],
                       [0.95,  R*2, R*2],   # the centre colour value is at 0.25 of the normalise vmax
                       [1.0,  1.0, 1.0]],
             'green': [[0.0,  G, G],
                       [0.95,  G*2, G*2],
                       [1.0,  1.0, 1.0]],
