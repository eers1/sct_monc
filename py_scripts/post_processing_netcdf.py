import xarray as xr
import pandas as pd
import cloud_lib as cfl
from multiprocessing import Pool
import re

def create_dataset(member_key, ice=False):
    sct_dir = "/gws/nopw/j04/carisma/eers/sct"

    if ice:
        key = "ice"
    else:
        r_key = re.compile('(\D*)(\d*)')
    ...
    total_lwp, cloudy_lwp, cloud_frac, times, lwp_mask_2d = cfl.lwp_cloud(ds)
    
    print('Calculating delta theta threshold')
    deltheta_thresh_heights = cfl.deltheta_thresh_timeseries(ds)
    ...
    post_processed_ds.to_netcdf(f"{sct_dir}/{key}/{member_key}/sct_{member_key}_pp.nc",mode="w")
...
def main():
    ...
    # index_range = range(2,12)
    # key_iterator = [f"{key}{index}" for index in index_range]
    # for member_key in key_iterator:
    #     create_dataset(member_key)

    key_iterator = ["em0_0", "em0_1", "em0_3", "em0_4", 
                    "em34_1", "em34_2", "em34_3", "em34_4",
                    "val19_1", "val19_2", "val19_3", "val19_4",
                    "xtra0_1", "xtra0_2", "xtra0_3", "xtra0_4"]
    for member_key in key_iterator:
